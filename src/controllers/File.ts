...
export async function uploadFile(request: Request, response: Response, next: NextFunction) {
	...
	try {
		await FileManagement.get().uploadFile(fileMetadata, `${bucketPrefix}-${trip.id}`, file.data);
	}
	catch(error) {
		return next(error);
	}
...
export async function getFile(request: Request, response: Response, next: NextFunction) {
	...
	try {
		res = await FileManagement.get().getFile(metadata.id.toString(), `${bucketPrefix}-${trip.id}`);
	}
	catch(error) {
		return next(error);
	}
...
export async function deleteFile(request: Request, response: Response) {
	...
	await FileManagement.get().deleteFile(metadata.id.toString(), `${bucketPrefix}-${trip.id}`);
